from __future__ import annotations

import typer

from . import __version__
from .data.cli import data_app
from .voices.cli import voices_app

app = typer.Typer(add_completion=False, help="radiotalk — ATC language tooling.")
app.add_typer(data_app, name="data")
app.add_typer(voices_app, name="voices")


def _add_audio_typer() -> None:
    """Audio subcommand requires the `audio` extra (heavy: torch,
    transformers) plus the TTS engine of the day. Import lazily so
    `radiotalk data` / `radiotalk voices` work without it installed.
    """
    try:
        from .audio.cli import audio_app
    except ImportError:
        return
    app.add_typer(audio_app, name="audio")


def _add_radio_typer() -> None:
    try:
        from .radio.cli import radio_app
    except ImportError:
        return
    app.add_typer(radio_app, name="radio")


_add_audio_typer()
_add_radio_typer()


@app.command()
def version() -> None:
    """Print the installed radiotalk version."""
    typer.echo(__version__)


if __name__ == "__main__":  # pragma: no cover
    app()
